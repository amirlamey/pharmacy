...
def execute(filters=None):
    ...
    if branchs or main_branch:
        for row in data:
            ...
            if main_branch:
                row[f"{main_branch}stock"] = get_stock_balance(item_code, main_branch, to_date) / conversion_factor
                main_stock = row[f"{main_branch}stock"]
                ...
                """, as_dict=1)
                row[f"{main_branch}sales"] = sold_qty[0]['qty'] / conversion_factor if sold_qty[0]['qty'] else 0

            for branch in branchs:
                stock_balance = get_stock_balance(item_code, branch, to_date) / conversion_factor
                row[f"{branch}stock"] = stock_balance if stock_balance else 0
                sold_qty2 = frappe.db.sql(f"""
                select sum(sii.stock_qty) qty
                from `tabSales Invoice Item` sii, `tabSales Invoice` si
                where sii.parent = si.name
                and si.docstatus = 1
                ...
                row[f"{branch}sales"] = sold_qty2[0]['qty'] / conversion_factor if sold_qty2[0]['qty'] else 0

                avsales = row[f"{branch}sales"] / days_diff 
                initial_reorder_qty = (avsales * no_of_days) - row[f"{branch}stock"]
                if initial_reorder_qty < 0:
                    initial_reorder_qty = 0

                # Adjust the reorder quantity based on the main warehouse stock
                if main_stock > 0:
                    if initial_reorder_qty <= main_stock:
                        row[f"{branch}reorder"] = initial_reorder_qty
                        main_stock -= initial_reorder_qty
                    else:
                        row[f"{branch}reorder"] = main_stock
                        main_stock = 0
                else:
                    row[f"{branch}reorder"] = initial_reorder_qty

            if main_stock != 0 or any(row[f"{branch}reorder"] > 0 for branch in branchs):
